...
function BuilderSettingsContent({
	...
	useEffect(() => {
		...
		return webSocketService.onPlatformJobUpdate(jobId, (job) => {
			...
			if (job.status === "succeeded") {
				...
				toast.error(
					job.error?.message ?? "Builder runner setup failed",
				);
			}
		});
	...
	useEffect(() => {
		if (!platformJobQuery.data || !terminal(platformJobQuery.data.status))
			return;
		void queryClient.invalidateQueries({ queryKey: setupKey });
	...
	const saveMutation = useMutation({
			...
			const payload: BuilderRunnerConfigSave = {
				...
				cloudflare:
					draft.provider === "cloudflare"
						...
						: null,
				local: draft.provider === "local" ? {} : null,
			};
		...
		onSuccess: async (config) => {
			...
			toast.success(
				config.enabled ? "Builder enabled" : "Runner settings saved",
			);
		},
		onError: (error: Error) => {
			setDraft((current) => ({
	...
	const draftDirty = Boolean(
		...
		(draft.provider === "cloudflare" &&
			(draft.accountId.trim() !== (config.cloudflare?.account_id ?? "") ||
				Boolean(draft.apiToken.trim()))),
	);
	...
	return (
		<div className="space-y-6 pb-8">
				...
				<div className="grid gap-6 p-5 sm:p-6 lg:grid-cols-[1fr_260px]">
					<div>
						<div className="flex flex-wrap items-center gap-2">
							<Badge
								variant={
									readiness?.ready ? "secondary" : "outline"
								}
								className={cn(
									"gap-1.5",
									readiness?.ready &&
						...
						<p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
							Bifrost coordinates durable jobs in the existing
							scheduler. Run them in the built-in Worker for local
							development, or let Cloudflare start an isolated
							container only while work is active. Neither option
							adds a public port or per-app DNS record.
						</p>
					</div>
			...
			<section className="space-y-4 rounded-3xl border bg-card p-5 sm:p-6">
				<div>
					...
					<p className="mt-1 text-sm text-muted-foreground">
						Both options use the same provider-neutral job and
						callback contract.
					</p>
				</div>
				<div className="grid gap-3 sm:grid-cols-2">
					...
					<ProviderChoice
						...
						detail="Use the Worker already included with Bifrost. Best for local development and smaller installs."
						onClick={() =>
							setDraft((current) => ({
								...current,
								provider: "local",
				...
				{draft.provider === "cloudflare" ? (
					<div className="grid gap-4 sm:grid-cols-2">
						...
						<p className="sm:col-span-2 flex items-start gap-2 rounded-2xl bg-muted/35 p-3 text-xs leading-5 text-muted-foreground">
							...
							{setup.cloudflare_permissions?.join(", ") ||
								"Workers Scripts Write and Workers Containers Write"}
							. Bifrost encrypts it and never returns it to the
							browser.
						</p>
						<div className="sm:col-span-2 flex items-start gap-3 rounded-2xl border bg-muted/20 p-3">
							...
							<div className="min-w-0">
								...
								<p className="mt-1 text-xs leading-5 text-muted-foreground">
									Bifrost uses its existing address
									automatically. No additional hostname, DNS
									record, or forwarded port is needed.
								</p>
							</div>
						</div>
					</div>
					...
					<div className="flex items-start gap-3 rounded-2xl border bg-muted/20 p-4">
						...
						<div>
							<p className="text-sm font-medium">
								No runner settings required
							</p>
							<p className="mt-1 text-xs leading-5 text-muted-foreground">
								Builder turns and app builds use the existing
								Bifrost Worker, RabbitMQ, and the same Pydantic
								AI harness as Chat. Saving this option only
								selects the local execution backend.
							</p>
						</div>
					</div>
				)}
				...
				<div className="flex flex-wrap items-center justify-between gap-3 border-t pt-4">
					{draft.provider === "cloudflare" ? (
						...
						<p className="text-xs text-muted-foreground">
							Uses the existing Worker container
						</p>
					)}
					<Button
			...
			<section className="grid gap-4 rounded-3xl border bg-card p-5 sm:p-6 lg:grid-cols-[1fr_auto] lg:items-center">
				<div>
					<h3 className="text-lg font-semibold">
						2. Deploy and verify the runner
					</h3>
					<p className="mt-1 text-sm text-muted-foreground">
						{draft.provider === "cloudflare"
							? "Bifrost deploys the managed control script, starts one real container, and reports the self-test here live."
							: "Bifrost sends a probe through RabbitMQ and verifies that the existing Worker can receive Builder jobs."}
					</p>
					{draftDirty ? (
						<p
							className="mt-3 flex items-center gap-2 text-xs font-medium text-amber-700 dark:text-amber-300"
							role="status"
						>
							<AlertTriangle className="h-3.5 w-3.5" />
							Save these changes before testing the runner.
						</p>
					) : null}
					{provisioningJob ? (
						<div
						...
						>
							<div className="flex items-center justify-between text-xs">
								<span>
									{provisioningJob.progress.phase ??
										"Provisioning Builder runner"}
								</span>
								<span>
									{Math.round(
										provisioningJob.progress.percent ?? 0,
									)}
									%
								</span>
							</div>
							<Progress
								value={provisioningJob.progress.percent ?? 0}
			...
			<section className="flex flex-col gap-4 rounded-3xl border bg-card p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
				<div>
					<h3 className="text-lg font-semibold">
						3. Enable Builder for users
					</h3>
					<p className="mt-1 text-sm text-muted-foreground">
						Build stays hidden from ordinary users until AI and the
						runner are connected and this switch is on.
					</p>
				</div>
				...
				<div className="flex flex-wrap items-start justify-between gap-3">
					<div>
						...
						<p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">
							Builders see their enforced turn budget while they
							work. Administrators can trace AI spend to the user
							and customer organization that created it.
						</p>
					</div>
...
function ReadinessChecklist({
	...
	const checks = [
		...
		{
			label: "Runner credentials",
			ready: readiness?.credentials_configured,
		},
	...
	return (
		<section className="grid gap-px overflow-hidden rounded-3xl border bg-border sm:grid-cols-5">
			{checks.map((check) => (
				<div
					key={check.label}
					className="flex min-h-20 items-center gap-3 bg-card p-4"
				>
					{check.ready ? (
