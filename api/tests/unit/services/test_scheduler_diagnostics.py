...
async def clean_diagnostics(db_session: AsyncSession) -> None:
    await db_session.execute(delete(SystemDiagnosticLog))
    await db_session.execute(delete(SchedulerTaskRun))
    await db_session.execute(delete(SchedulerTaskState))
    await db_session.execute(delete(SchedulerReplica))
    await db_session.execute(delete(SchedulerLease))
...
async def test_records_replica_schedule_run_and_curated_logs(
    ...
    run_id = await diagnostics.start_scheduler_run(
        "oauth_token_refresh", "scheduler-a"
    )
    ...
    replica = await db_session.get(SchedulerReplica, "scheduler-a")
    assert replica is not None
    assert replica.memory_current_bytes == 600
    assert replica.memory_limit_bytes == 1000

    state = await db_session.get(SchedulerTaskState, "oauth_token_refresh")
    ...
    logs = (
        await db_session.execute(
            select(SystemDiagnosticLog).order_by(SystemDiagnosticLog.id)
        )
    ).scalars().all()
...
async def test_scheduler_snapshot_explains_capacity_pressure(
    ...
    assert response.leader.healthy
    assert response.leader.owner_id == "scheduler-a"
    assert response.capacity.replicas_online == 1
    assert response.capacity.slots_total == 1
    assert response.capacity.max_memory_utilization_percent == 60
    assert response.replicas[0].is_leader
