...
def test_hardlinked_file_read_is_rejected(tmp_path: Path, limits: WorkspaceLimits) -> None:
    root = tmp_path / "ws"
    root.mkdir()
    original = root / "a.txt"
...
def test_write_rejects_over_file_count_limit(tmp_path: Path) -> None:
    root = tmp_path / "ws"
    root.mkdir()
...
def test_search_text_finds_matches_and_respects_glob(workspace: WorkspaceRoot) -> None:
...
def test_extract_rejects_symlink_member(tmp_path: Path, limits: WorkspaceLimits) -> None:
...
def test_extract_rejects_duplicate_members(tmp_path: Path, limits: WorkspaceLimits) -> None:
...
def test_extract_rejects_oversized_member(tmp_path: Path, limits: WorkspaceLimits) -> None:
    archive = _write_zip(
...
def test_extract_streamed_byte_budget_stops_oversized_expansion(tmp_path: Path) -> None:
    archive = _write_zip(
