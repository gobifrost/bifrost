...
async def test_get_preserves_non_auth_api_failures(status_code: int) -> None:
    client = AsyncMock()
    client.engine_request.return_value = _response(status_code)
...
async def test_get_surfaces_unauthorized_as_authentication_error() -> None:
    client = AsyncMock()
    client.engine_request.return_value = _response(401, "CLI session expired")
...
async def test_mapping_reads_preserve_api_failures(method_name: str) -> None:
    client = AsyncMock()
    client.engine_request.return_value = _response(503)
