...
async def test_capture_copies_loose_agent_bundle_from_agent_storage(monkeypatch) -> None:
    repo = _FakeRepo({
        "agents/helper/SKILL.md": b"# Helper\n",
        "agents/helper/references/guide.md": b"Guide",
        "agents/helper/scripts/example.py": b"inert = True\n",
        "agents/helper/private.txt": b"not part of portable bundle",
    })

    class _EmptyScopedStorage:
        async def list(self, prefix: str = "") -> list[str]:
            return []

    monkeypatch.setattr(
        "src.services.solutions.storage.SolutionStorage",
        lambda _solution_id: _EmptyScopedStorage(),
    )
    monkeypatch.setattr(
        "src.services.agent_skill_storage.AgentSkillStorage",
        lambda _agent_id: repo,
    )
    service = SolutionCaptureService(db=None, repo=repo)  # type: ignore[arg-type]

    files = await service._agent_bundle_files(
        uuid.uuid4(),
        [{"id": uuid.uuid4(), "bundle_path": "agents/helper"}],
    )
...
async def test_capture_rejects_bundle_without_skill_entrypoint(monkeypatch) -> None:
    repo = _FakeRepo({
        "agents/helper/references/guide.md": b"Guide",
    })
    ...
    with pytest.raises(ValueError, match="missing SKILL.md"):
        await service._agent_bundle_files(
            uuid.uuid4(),
            [{"id": uuid.uuid4(), "bundle_path": "agents/helper"}],
        )


async def test_capture_copies_git_synced_bundle_from_repo(monkeypatch) -> None:
    repo = _FakeRepo(
        {
            "agents/helper/SKILL.md": b"# Helper\n",
            "agents/helper/references/guide.md": b"Guide",
    ...
    assert files["agents/helper/references/guide.md"] == b"Guide"
