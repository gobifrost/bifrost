...
ALLOW_LIST_INLINE_ORG: set[tuple[str, str, str]] = {
    ...
    ('routers/claims.py', 'stmt = stmt.where(ClaimORM.organization_id == boundary.organization_id)', 'claims inline lookups; phase 6 migrates via CustomClaimRepository'),
    ('routers/claims.py', 'stmt = stmt.where(ClaimORM.organization_id == requested)', 'claims inline lookups; phase 6 migrates via CustomClaimRepository'),
    ...
    ('routers/events.py', 'and boundary.organization_id == organization_id', 'event scope filter; identity resolution'),
    ('routers/export_import.py', 'criteria.append(model.organization_id.is_(None))', 'manifest sync inline; phase 8 follow-up'),
    ('routers/export_import.py', 'criteria.append(model.organization_id.in_(concrete_org_ids))', 'manifest sync inline; phase 8 follow-up'),
    ('routers/export_import.py', 'KnowledgeStore.organization_id == org_id', 'manifest sync inline; phase 8 follow-up'),
    ('routers/export_import.py', 'KnowledgeStore.organization_id.is_(None)', 'manifest sync inline; phase 8 follow-up'),
    ('routers/export_import.py', 'IntegrationMapping.organization_id == org_id', 'manifest sync inline; phase 8 follow-up'),
    ('routers/export_import.py', 'IntegrationMapping.organization_id.is_(None)', 'manifest sync inline; phase 8 follow-up'),
    ('routers/export_import.py', 'Config.organization_id == mapping.organization_id', 'manifest sync mapping lookup; phase 8 follow-up'),
    ('routers/export_import.py', 'else Config.organization_id.is_(None),', 'manifest sync global mapping lookup; phase 8 follow-up'),
    ('routers/export_import.py', 'Config.organization_id.is_(None),', 'manifest sync global config lookup; phase 8 follow-up'),
    ('routers/export_import.py', 'existing_query = existing_query.where(Table.organization_id == org_id)', 'manifest sync exact-scope table lookup; phase 8 follow-up'),
    ('routers/export_import.py', 'existing_query = existing_query.where(Table.organization_id.is_(None))', 'manifest sync global table lookup; phase 8 follow-up'),
    ('routers/export_import.py', 'existing_query = existing_query.where(Config.organization_id == org_id)', 'manifest sync exact-scope config lookup; phase 8 follow-up'),
    ('routers/export_import.py', 'existing_query = existing_query.where(Config.organization_id.is_(None))', 'manifest sync global config lookup; phase 8 follow-up'),
    ('routers/integrations.py', 'IntegrationMapping.organization_id == boundary.organization_id', 'integration mapping list filter (exact scope, NOT cascade); IntegrationRepository is not an OrgScopedRepository'),
    ('routers/integrations.py', 'IntegrationMapping.organization_id == organization_id', 'integration mapping exact-scope lookup; phase 8 follow-up'),
    ('routers/integrations.py', 'IntegrationMapping.organization_id == org_id,', 'integration mapping exact-scope lookup; phase 8 follow-up'),
    ('routers/integrations.py', 'ConfigModel.organization_id.is_(None),', 'integration global config lookup; phase 8 follow-up'),
    ('routers/integrations.py', 'ConfigModel.organization_id == organization_id,', 'integration exact-scope config lookup; phase 8 follow-up'),
    ('routers/integrations.py', 'ConfigModel.organization_id == org_id,', 'integration exact-scope config lookup; phase 8 follow-up'),
    ('routers/integrations.py', 'and boundary.organization_id == organization_id', 'integration mapping boundary filter; phase 8 follow-up'),
    ('routers/integrations.py', 'if mapping.organization_id == boundary.organization_id', 'integration mapping boundary filter; phase 8 follow-up'),
    ('routers/knowledge_sources.py', 'return KnowledgeStore.organization_id.is_(None)', 'knowledge sources inline cascade; phase 6 migrates'),
    ('routers/knowledge_sources.py', 'KnowledgeStore.organization_id == boundary.organization_id,', 'knowledge sources inline cascade; phase 6 migrates'),
    ('routers/knowledge_sources.py', 'KnowledgeStore.organization_id.is_(None),', 'knowledge sources inline cascade; phase 6 migrates'),
    ('routers/knowledge_sources.py', 'return KnowledgeStore.organization_id.in_(', 'knowledge sources inline cascade; phase 6 migrates'),
    ('routers/knowledge_sources.py', 'return KnowledgeNamespaceRole.organization_id.is_(None)', 'knowledge sources inline cascade; phase 6 migrates'),
    ('routers/knowledge_sources.py', 'KnowledgeNamespaceRole.organization_id == boundary.organization_id,', 'knowledge sources inline cascade; phase 6 migrates'),
    ('routers/knowledge_sources.py', 'KnowledgeNamespaceRole.organization_id.is_(None),', 'knowledge sources inline cascade; phase 6 migrates'),
    ('routers/knowledge_sources.py', 'return KnowledgeNamespaceRole.organization_id.in_(', 'knowledge sources inline cascade; phase 6 migrates'),
    ('routers/knowledge_sources.py', 'and boundary.organization_id == organization_id', 'knowledge sources inline cascade; phase 6 migrates'),
    ('routers/mcp_connections.py', 'query = query.where(MCPConnection.organization_id.is_(None))', 'MCP connection org filter; phase 6 migrates'),
    ('routers/mcp_connections.py', '.where(MCPConnection.organization_id.in_(visible_orgs))', 'MCP connection org filter; phase 6 migrates'),
    ('routers/mcp_servers.py', 'and server.organization_id == boundary.organization_id', 'MCP server org filter; phase 6 migrates'),
    ('routers/mcp_servers.py', '(MCPServer.organization_id.is_(None))', 'MCP server org filter; phase 6 migrates'),
    ('routers/mcp_servers.py', '| (MCPServer.organization_id.in_(visible_orgs))', 'MCP server org filter; phase 6 migrates'),
    ('routers/metrics.py', 'Organization, ExecutionMetricsDaily.organization_id == Organization.id', 'ExecutionMetricsDaily identity-entity filter (permanent)'),
    ('routers/platform_jobs.py', 'return job.organization_id == boundary.organization_id', 'platform jobs boundary filter; permanent'),
    ('routers/platform_jobs.py', 'admitted_filter = PlatformJob.organization_id.is_(None)', 'platform jobs boundary filter; permanent'),
    ('routers/platform_jobs.py', 'admitted_filter = PlatformJob.organization_id == boundary.organization_id', 'platform jobs boundary filter; permanent'),
    ('routers/platform_jobs.py', 'admitted_filter = PlatformJob.organization_id.in_(managed_org_ids)', 'platform jobs boundary filter; permanent'),
    ('routers/policy_rules.py', 'if row.organization_id == organization_id', 'policy rule org filter; permanent'),
    ('routers/roi_reports.py', 'Organization, ExecutionMetricsDaily.organization_id == Organization.id', 'identity-entity scope filter (permanent)'),
    ('routers/roles.py', 'OrganizationGroupMembership.organization_id.in_(managed_org_ids)', 'roles org membership boundary; permanent'),
    ('routers/roles.py', 'boundary_row.organization_id.in_(managed_org_ids),', 'roles org membership boundary; permanent'),
    ('routers/roles.py', 'boundary_row.organization_id == boundary.organization_id,', 'roles org membership boundary; permanent'),
    ...
    ('routers/solutions.py', 'return SolutionORM.organization_id.is_(None)', 'solution scope filter; permanent'),
    ('routers/solutions.py', 'return SolutionORM.organization_id == boundary.organization_id', 'solution scope filter; permanent'),
    ('routers/solutions.py', 'return SolutionORM.organization_id.in_(', 'solution scope filter; permanent'),
    ('routers/solutions.py', 'Config.organization_id == sol.organization_id', 'entities endpoint: install-scoped config-key existence read for value_set status (NOT cascade)'),
    ('routers/usage_limits.py', 'and boundary.organization_id == organization_id', 'usage limit boundary filter; permanent'),
    ('routers/usage_reports.py', 'AIUsage.organization_id == filter_org_id', 'identity-entity scope filter (permanent)'),
    ...
    ('routers/usage_reports.py', 'org_query = org_query.where(AIUsage.organization_id == filter_org_id)', 'identity-entity scope filter (permanent)'),
    ('routers/users.py', 'query = query.where(UserORM.organization_id == boundary.organization_id)', 'User identity-entity filter (permanent)'),
    ('routers/users.py', 'query = query.where(UserORM.organization_id.in_(managed_org_ids))', 'User identity-entity filter (permanent)'),
    ('routers/users.py', 'query = query.where(UserORM.organization_id.is_(None))', 'User global identity filter (permanent)'),
    ('routers/websocket.py', 'return boundary.organization_id == organization_id', 'websocket table subscription filter; phase 6 migrates'),
    ('routers/websocket.py', 'stmt = stmt.where(TableOrm.organization_id.is_(None))', 'websocket table subscription filter; phase 6 migrates'),
    ('routers/websocket.py', '(TableOrm.organization_id == boundary.organization_id)', 'websocket table subscription filter; phase 6 migrates'),
    ('routers/websocket.py', '| TableOrm.organization_id.is_(None)', 'websocket table subscription filter; phase 6 migrates'),
    ('routers/websocket.py', '(TableOrm.organization_id == user.organization_id)', 'legacy websocket exact-scope filter; phase 6 migrates'),
    ('routers/websocket.py', 'TableOrm.organization_id.in_(', 'websocket table subscription filter; phase 6 migrates'),
    ('routers/workflow_keys.py', 'return Workflow.organization_id.is_(None)', 'workflow keys org boundary; permanent'),
    ('routers/workflow_keys.py', 'return Workflow.organization_id == boundary.organization_id', 'workflow keys org boundary; permanent'),
    ('routers/workflow_keys.py', 'return Workflow.organization_id.in_(', 'workflow keys org boundary; permanent'),
    ('routers/workflows.py', 'Application.organization_id == org_filter', 'workflows inline cascade; phase 6 migrates'),
    ('routers/workflows.py', 'forms_query = forms_query.where(Form.organization_id == org_filter)', 'workflows related-form exact-scope lookup; phase 6 migrates'),
    ('routers/workflows.py', 'agents_query = agents_query.where(Agent.organization_id == org_filter)', 'workflows related-agent exact-scope lookup; phase 6 migrates'),
}
