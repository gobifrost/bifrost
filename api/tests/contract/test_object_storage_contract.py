...
def _app_storage_backends() -> list[AppStorageBackend]:
    backends: list[AppStorageBackend] = []

    bucket = os.getenv("BIFROST_STORAGE_CONTRACT_S3_BUCKET")
    endpoint_url = os.getenv("BIFROST_STORAGE_CONTRACT_S3_ENDPOINT_URL")
    access_key_id = os.getenv("BIFROST_STORAGE_CONTRACT_S3_ACCESS_KEY_ID")
    secret_access_key = os.getenv("BIFROST_STORAGE_CONTRACT_S3_SECRET_ACCESS_KEY")
    if all((bucket, endpoint_url, access_key_id, secret_access_key)):
        backends.append(
            AppStorageBackend(
                name="s3",
                settings_kwargs={
                    ...
                    "s3_region": os.getenv(
                        "BIFROST_STORAGE_CONTRACT_S3_REGION", "auto"
                    ),
                },
            )
        )
    ...
    container = os.getenv("BIFROST_STORAGE_CONTRACT_AZURE_BLOB_CONTAINER")
    account_key = os.getenv("BIFROST_STORAGE_CONTRACT_AZURE_BLOB_ACCOUNT_KEY")
    if all((account_url, container, account_key)):
        backends.append(
            AppStorageBackend(
                name="azure-blob",
                settings_kwargs={
...
@pytest_asyncio.fixture(
    params=_app_storage_backends(),
    ids=lambda backend: backend.name,
)
async def app_storage_service(request: pytest.FixtureRequest):
    if not _app_storage_backends():
        pytest.skip("real object storage env is required for app storage tests")
    ...
    settings = Settings(
        secret_key="x" * 32,
        environment="testing",
        **request.param.settings_kwargs,
    )
    service = AppStorageService(settings=settings)
    if settings.object_storage_provider == "azure_blob":
        from azure.core.exceptions import ResourceExistsError

        await service._storage._ensure_client()
        try:
            await service._storage._container_client.create_container()
        except ResourceExistsError:
            pass

    async def no_cache_invalidation(app_id: str) -> None:
        del app_id

    service.invalidate_render_cache = no_cache_invalidation  # type: ignore[method-assign]
    try:
        yield service
    finally:
        close = getattr(service._storage, "close", None)
...
@pytest.mark.asyncio
async def test_app_storage_publish_uses_selected_real_backend(app_storage_service):
    app_id = f"contract-{uuid4().hex}"

    await app_storage_service.write_preview_file(
        app_id,
        "index.js",
        b"export default 'preview';",
    )
    ...
    published = await app_storage_service.publish(app_id)

    assert published == 2
    assert (
        await app_storage_service.read_file(app_id, "live", "index.js")
        == b"export default 'preview';"
    )
    assert (
        await app_storage_service.read_file(app_id, "live", "nested/data.json")
        == b'{"ok":true}'
    )


@pytest.mark.asyncio
async def test_app_storage_live_write_uses_selected_real_backend(app_storage_service):
    app_id = f"contract-{uuid4().hex}"

    await app_storage_service.write_live_file(
        app_id,
        "entry.js",
        b"console.log('live');",
    )
    ...
    assert (
        await app_storage_service.read_file(app_id, "live", "entry.js")
        == b"console.log('live');"
    )


@pytest.mark.asyncio
async def test_app_storage_sync_preview_uses_selected_real_backend(app_storage_service):
    from src.services.repo_storage import RepoStorage

    app_id = f"contract-{uuid4().hex}"
    source_dir = f"apps/{app_id}"
    repo_storage = RepoStorage(settings=app_storage_service._settings)

    await repo_storage.write(
        f"{source_dir}/index.js",
        b"export default 'from repo';",
    )
    await repo_storage.write(
        f"{source_dir}/nested/data.json",
        b'{"from":"repo"}',
    )

    synced = await app_storage_service.sync_preview(app_id, source_dir)

    assert synced == 2
    assert (
        await app_storage_service.read_file(app_id, "preview", "index.js")
        == b"export default 'from repo';"
    )
    assert (
        await app_storage_service.read_file(app_id, "preview", "nested/data.json")
        == b'{"from":"repo"}'
    )
