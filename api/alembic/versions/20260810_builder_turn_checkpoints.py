...
or cancelled turns can retain an inert workspace checkpoint without advancing
the Solution's current revision. Each turn also binds its exact Chat message.
"""
...
def upgrade() -> None:
    ...
    op.create_foreign_key(
        "fk_solution_builder_turns_user_message_id",
        "solution_builder_turns",
        "messages",
        ["user_message_id"],
        ["id"],
    ...
    op.create_index(
        "ix_solution_builder_turns_user_message_id",
        "solution_builder_turns",
        ["user_message_id"],
    )
...
def downgrade() -> None:
    ...
    op.drop_column("solution_builder_turns", "user_message_id")
