...
class ClaimQuery(BaseModel):
    ...
    where: dict | None = Field(
        default=None, description="Filter AST (same shape as policies)"
    )
    select: str = Field(
        min_length=1, description="Column or JSON path on the source table"
    )
...
class ManifestOrganization(EntityCodec, BaseModel):
    ...
    def to_orm_values(self, dest: Destination) -> ImportFields:
        return ImportFields(
            direct={"id": self.id, "name": self.name, "is_active": self.is_active}
        )
...
class ManifestRole(EntityCodec, BaseModel):
    ...
    @classmethod
    def normalize_legacy_capability_fields(cls, data: Any) -> Any:
        ...
        if legacy_scopes is not None:
            ...
            if capabilities is not None and list(capabilities) != translated_scopes:
                raise ValueError(
                    "Use either capabilities or matching legacy scopes, not both"
                )
...
class ManifestWorkflow(EntityCodec, BaseModel):
    ...
    name: str = Field(
        default="",
        description="MCP tool name; defaults to function_name on registration",
        **classify(FieldClass.CONTENT),
    )
    path: str = Field(
        description="Relative path to Python file (e.g. 'workflows/onboard.py')",
        **classify(FieldClass.CONTENT, match_key=True),
    )
    function_name: str = Field(
        description="Python function name decorated with @workflow/@tool/@data_provider",
        **classify(FieldClass.CONTENT, match_key=True),
    )
    type: str = Field(
        default="workflow",
        description="workflow | tool | data_provider",
        **classify(FieldClass.CONTENT),
    )
    ...
    roles: list[str] = Field(
        ...
        description="Role UUIDs that can access this workflow",
        **classify(FieldClass.ENVIRONMENT, install_view="keep_empty_list"),
    )
    ...
    access_level: str = Field(
        ...
        description="role_based | authenticated | everyone | public",
        **classify(FieldClass.CONTENT),
    )
    ...
    allowed_methods: list[str] = Field(
        ...
        description="Allowed HTTP methods for endpoint workflows",
        **classify(FieldClass.CONTENT),
    )
    ...
    public_endpoint: bool = Field(
        default=False,
        description="Allow unauthenticated API access",
        **classify(FieldClass.CONTENT),
    )
    description: str | None = Field(
        default=None, description="Workflow description", **classify(FieldClass.CONTENT)
    )
    ...
    tags: list[str] = Field(
        ...
        **classify(FieldClass.CONTENT, install_view="keep_empty_list"),
    )
    ...
    def from_row(cls, wf, *, roles: list[str] | None = None) -> "ManifestWorkflow":
        ...
        return cls(
            ...
            if wf.timeout_seconds is not None
            else 1800,
...
class ManifestForm(EntityCodec, BaseModel):
    ...
    id: str = Field(
        description="Form UUID", **classify(FieldClass.IDENTITY, import_owner="indexer")
    )
    name: str = Field(
        default="",
    ...
    path: str | None = Field(
        ...
        **classify(
            FieldClass.CONTENT, install_view="drop"
        ),  # deprecated; install omits it
    )
    # -- Environment-specific fields (NOT portable; do not include when sharing) --
    organization_id: str | None = Field(
        default=None,
        description="Org UUID (null = global)",
        **classify(FieldClass.ENVIRONMENT, import_owner="restamp"),
    )
    # roles is ENVIRONMENT but install carries the access grant the deployer re-binds.
    roles: list[str] = Field(
        default_factory=list,
        description="Role UUIDs that can access this form",
        **classify(FieldClass.ENVIRONMENT, install_view="keep_empty_list"),
    )
    role_names: list[str] | None = Field(
        default=None,
    ...
    access_level: str | None = Field(
        default=None,
        description="role_based | authenticated | everyone | public",
        **classify(FieldClass.CONTENT, import_owner="restamp"),
    )
    ...
    confirmation_markdown: str | None = Field(
        default=None,
        description="Markdown shown after embedded submission",
        **classify(FieldClass.CONTENT, import_owner="indexer"),
    )
    workflow_id: str | None = Field(
        default=None,
        description="Workflow UUID to execute on submit",
        **classify(FieldClass.REFERENCE, import_owner="indexer"),
    )
    launch_workflow_id: str | None = Field(
        default=None,
        description="Workflow UUID to run on form load",
        **classify(FieldClass.REFERENCE, import_owner="indexer"),
    )
    default_launch_params: dict | None = Field(
        default=None,
        description="Default params for launch workflow",
        **classify(FieldClass.CONTENT, import_owner="indexer"),
    )
    allowed_query_params: list[str] | None = Field(
        default=None,
        description="Form fields populatable via URL query params",
        **classify(FieldClass.CONTENT, import_owner="indexer"),
    )
    form_schema: dict | None = Field(
        default=None,
        description="Form schema (fields list, etc.)",
        **classify(FieldClass.CONTENT, import_owner="indexer"),
    )

    @classmethod
    def from_row(
        cls, form, *, roles: list[str] | None = None, fields: list | None = None
    ) -> "ManifestForm":
...
class ManifestAgent(EntityCodec, BaseModel):
    ...
    - indexer_content: id, name (always), + description/system_prompt/channels/
      ...
      mcp_connection_ids/llm_profile/llm_max_tokens (non-empty lists only,
    ...
    id: str = Field(
        description="Agent UUID", **classify(FieldClass.IDENTITY, import_owner="direct")
    )
    name: str = Field(
        default="",
    ...
    roles: list[str] = Field(
        default_factory=list,
        description="Role UUIDs that can access this agent",
        **classify(FieldClass.ENVIRONMENT, install_view="keep"),
    )
    role_names: list[str] | None = Field(
        default=None,
        description="Role display names (used by portable bundles; resolved to UUIDs on import)",
        **classify(
            FieldClass.ENVIRONMENT,
            keep_on_portable=True,
            install_view="keep_empty_list",
        ),
    )
    access_level: str | None = Field(
        default=None,
    ...
    channels: list[str] = Field(
        default_factory=list,
        description="Channels the agent runs on (chat, email, …)",
        **classify(FieldClass.CONTENT, import_owner="indexer"),
    )
    tool_ids: list[str] = Field(
        default_factory=list,
        description="Workflow UUIDs exposed as tools",
        **classify(FieldClass.REFERENCE, import_owner="indexer"),
    )
    delegated_agent_ids: list[str] = Field(
        default_factory=list,
        description="Agent UUIDs this agent can delegate to",
        **classify(FieldClass.REFERENCE, import_owner="indexer"),
    )
    knowledge_sources: list[str] = Field(
        default_factory=list,
        description="Knowledge namespaces searchable via RAG",
        **classify(
            FieldClass.CONTENT, import_owner="indexer", install_view="keep_empty_list"
        ),
    )
    system_tools: list[str] = Field(
        default_factory=list,
        description="System tool names enabled (e.g. 'bifrost_execute_workflow')",
        **classify(
            FieldClass.CONTENT, import_owner="indexer", install_view="keep_empty_list"
        ),
    )
    ...
    llm_profile: str | None = Field(
        default=None,
        description="Model profile name (null = default profile assignment)",
        **classify(FieldClass.REFERENCE, import_owner="indexer"),
    )
    llm_max_tokens: int | None = Field(
        default=None,
        description="Override LLM max tokens (null = global default)",
        **classify(FieldClass.CONTENT, import_owner="indexer"),
    )
    ...
    max_token_budget: int | None = Field(
        default=None,
    ...
    def from_row(
        ...
        *,
        ...
        return cls(
            ...
            if agent.access_level
            else "role_based",
            description=agent.description,
            system_prompt=agent.system_prompt,
            bundle_path=(
                agent.bundle_path
                if isinstance(getattr(agent, "bundle_path", None), str)
                else None
            ),
...
class ManifestApp(EntityCodec, BaseModel):
    ...
    path: str = Field(
        description="App source directory (e.g. 'apps/my-dashboard'), not app.yaml",
    ...
    name: str | None = Field(
        default=None, description="Display name", **classify(FieldClass.CONTENT)
    )
    description: str | None = Field(
        default=None, description="App description", **classify(FieldClass.CONTENT)
    )
    dependencies: dict[str, str] = Field(
        default_factory=dict,
        description="NPM packages {name: version}",
        **classify(FieldClass.CONTENT),
    )
    organization_id: str | None = Field(
        default=None,
    ...
    roles: list[str] = Field(
        default_factory=list,
        description="Role UUIDs that can access this app",
        **classify(FieldClass.ENVIRONMENT, install_view="keep"),
    )
    role_names: list[str] | None = Field(
        default=None,
    ...
    app_model: str = Field(
        default="inline_v1",
        description="Render model: inline_v1 | standalone_v2",
        **classify(FieldClass.CONTENT),
    )
    logo: str | None = Field(
        default=None,
...
class ManifestIntegrationConfigSchema(EntityCodec, BaseModel):
    ...
    key: str = Field(
        description="Config key name", **classify(FieldClass.CONTENT, match_key=True)
    )
    type: str = Field(
        description="string | int | bool | json | secret",
        **classify(FieldClass.CONTENT),
    )
    required: bool = Field(
        default=False,
        description="Whether this config must be set",
        **classify(FieldClass.CONTENT),
    )
    description: str | None = Field(
        default=None,
    ...
    position: int = Field(
        default=0, description="Display order in UI", **classify(FieldClass.CONTENT)
    )
...
class ManifestOAuthProvider(EntityCodec, BaseModel):
    ...
    display_name: str | None = Field(
        default=None, description="UI display name", **classify(FieldClass.CONTENT)
    )
    ...
    client_id: str = Field(
        ...
        description="OAuth client ID (set via UI)",
    ...
    token_url: str | None = Field(
        default=None, description="OAuth token endpoint", **classify(FieldClass.CONTENT)
    )
    token_url_defaults: dict | None = Field(
        default=None,
        description="Default params for token request",
        **classify(FieldClass.CONTENT),
    )
    scopes: list[str] = Field(
        default_factory=list, description="OAuth scopes", **classify(FieldClass.CONTENT)
    )
    redirect_uri: str | None = Field(
        default=None, description="OAuth redirect URI", **classify(FieldClass.CONTENT)
    )
...
class ManifestIntegrationMapping(EntityCodec, BaseModel):
    ...
    organization_id: str | None = Field(
        default=None,
        description="Org UUID this mapping belongs to",
        **classify(FieldClass.ENVIRONMENT),
    )
    entity_id: str = Field(
        description="External entity identifier (e.g. tenant ID)",
        **classify(FieldClass.REFERENCE),
    )
    ...
    oauth_token_id: str | None = Field(
        default=None,
        description="Linked OAuth token (set via UI)",
        **classify(FieldClass.REFERENCE),
    )
...
class ManifestIntegration(EntityCodec, BaseModel):
    ...
    entity_id: str | None = Field(
        default=None,
        description="Field name for entity identifier",
        **classify(FieldClass.REFERENCE),
    )
    entity_id_name: str | None = Field(
        default=None,
        description="Display label for entity ID field",
        **classify(FieldClass.CONTENT),
    )
    ...
    list_entities_data_provider_id: str | None = Field(
        default=None,
        description="Workflow UUID for entity dropdown",
        **classify(FieldClass.REFERENCE),
    )
    config_schema: list[ManifestIntegrationConfigSchema] = Field(
        default_factory=list,
        description="Configuration fields",
        **classify(FieldClass.CONTENT),
    )
    oauth_provider: ManifestOAuthProvider | None = Field(
        default=None,
        description="OAuth provider config",
        **classify(FieldClass.CONTENT),
    )
    mappings: list[ManifestIntegrationMapping] = Field(
        default_factory=list,
        description="Per-org entity mappings",
        **classify(FieldClass.CONTENT),
    )
    ...
    def from_row(
        ...
        *,
        ...
        return cls(
            ...
            mappings=[
                ManifestIntegrationMapping.from_row(im) for im in (mappings or [])
            ],
        )
...
class ManifestConfig(EntityCodec, BaseModel):
    ...
    integration_id: str | None = Field(
        default=None,
        description="Parent integration UUID (if integration config)",
        **classify(FieldClass.REFERENCE, match_key=True),
    )
    key: str = Field(
        description="Config key name", **classify(FieldClass.CONTENT, match_key=True)
    )
    ...
    value: object | None = Field(
        default=None,
        description="Config value (null for secret type)",
        **classify(FieldClass.CONTENT, predicate="config_value"),
    )
    ...
    def from_row(cls, cfg) -> "ManifestConfig":
        ...
        value = (
            None
            if (
                cfg.config_type == ConfigType.SECRET or str(cfg.config_type) == "secret"
            )
            else cfg.value
        )
        return cls(
...
class ManifestPolicyRule(EntityCodec, BaseModel):
    ...
    name: str = Field(
        description="Rule name (unique per domain+org)",
        **classify(FieldClass.CONTENT, match_key=True),
    )
    domain: str = Field(
        description="Policy domain: 'file' | 'table'",
        **classify(FieldClass.CONTENT, match_key=True),
    )
    description: str | None = Field(
        default=None,
    ...
    body: dict = Field(
        description="Rule body ({actions, when})", **classify(FieldClass.CONTENT)
    )
    organization_id: str | None = Field(
        default=None,
        description="Org UUID (null = global)",
    ...
    def to_orm_values(self, dest: Destination) -> ImportFields:
        if dest is not Destination.GIT_SYNC:
            raise NotImplementedError(
                f"ManifestPolicyRule has no install path; dest={dest}"
            )
        return ImportFields(
            direct={
                "id": self.id,
...
class ManifestSolutionConfigSchema(EntityCodec, BaseModel):
    ...
    required: bool = Field(
        default=False,
        description="Whether a value must be supplied at install time",
        **classify(FieldClass.CONTENT),
    )
    description: str | None = Field(
        default=None,
    ...
    default: object | None = Field(
        default=None,
        description="Default value used when none is supplied",
        **classify(FieldClass.CONTENT),
    )
    position: int = Field(
        default=0,
        description="Display ordering within the solution",
        **classify(FieldClass.CONTENT),
    )
...
class ManifestSolutionFile(EntityCodec, BaseModel):
    ...
    location: str = Field(
        description="File location bucket (e.g. 'shared')",
        **classify(FieldClass.CONTENT, match_key=True),
    )
    path: str = Field(
        description="File path relative to location (e.g. 'docs/foo.txt')",
        **classify(FieldClass.CONTENT, match_key=True),
    )
    sha256: str = Field(
        description="SHA-256 hex digest of file content", **classify(FieldClass.CONTENT)
    )
...
class ManifestCustomClaim(EntityCodec, BaseModel):
    ...
    name: str = Field(
        description="Claim name, unique per org",
        **classify(FieldClass.CONTENT, match_key=True),
    )
    description: str | None = Field(
        default=None,
    ...
    organization_id: str = Field(
        description="Org UUID", **classify(FieldClass.ENVIRONMENT, match_key=True)
    )
    type: Literal["list", "scalar"] = Field(
        default="list", description="list | scalar", **classify(FieldClass.CONTENT)
    )
    query: ClaimQuery = Field(
        description="Source table query that resolves the claim",
        **classify(FieldClass.CONTENT),
    )
...
class ManifestPolicy(BaseModel):
    ...
    name: str = Field(
        description="Unique policy name within the table",
        **classify(FieldClass.CONTENT, match_key=True),
    )
    description: str | None = Field(
        default=None,
...
class ManifestPolicyRef(BaseModel):
    ...
    ref: str = Field(
        alias="$ref", min_length=1, max_length=100, **classify(FieldClass.CONTENT)
    )
...
class ManifestTable(EntityCodec, BaseModel):
    ...
    description: str | None = Field(
        default=None, description="Table description", **classify(FieldClass.CONTENT)
    )
    organization_id: str | None = Field(
        default=None,
        description="Org UUID (null = global)",
    ...
    table_schema: dict | None = Field(
        default=None,
        alias="schema",
        description="Column definitions and validation hints",
        **classify(FieldClass.CONTENT),
    )
        ...
        if raw_policies:
            ...
            for p in raw_policies:
                ...
                if isinstance(p, dict) and ("$ref" in p or "ref" in p):
                    ...
                    if not ref_val:
                        raise ValueError(
                            f"malformed policy ref entry (null or empty $ref): {p!r}"
                        )
...
class ManifestFilePolicy(EntityCodec, BaseModel):
    ...
    def from_row(cls, file_policy) -> "ManifestFilePolicy":
        raw_policies = (
            file_policy.policies if isinstance(file_policy.policies, dict) else {}
        )
...
class ManifestEventSubscription(EntityCodec, BaseModel):
    ...
    target_type: str = Field(
        default="workflow",
        description="'workflow' or 'agent'",
        **classify(FieldClass.CONTENT),
    )
    workflow_id: str | None = Field(
        default=None,
        description="Workflow UUID to trigger (when target_type='workflow')",
        **classify(FieldClass.REFERENCE),
    )
    agent_id: str | None = Field(
        default=None,
        description="Agent UUID to run (when target_type='agent')",
        **classify(FieldClass.REFERENCE),
    )
    event_type: str | None = Field(
        default=None,
        description="Filter by event type (e.g. 'ticket.created')",
        **classify(FieldClass.CONTENT),
    )
    ...
    input_mapping: dict | None = Field(
        default=None,
        description="Map event fields to workflow params",
        **classify(FieldClass.CONTENT),
    )
    is_active: bool = Field(
        default=True,
        description="Enable/disable this subscription",
        **classify(FieldClass.ENVIRONMENT),
    )
    ...
    def to_orm_values(self, dest: Destination) -> ImportFields:
        raise NotImplementedError(
            "EventSubscription rows are built by the parent resolver/deploy"
        )
...
class ManifestEventSource(EntityCodec, BaseModel):
    ...
    name: str = Field(
        default="",
        description="Event source display name",
        **classify(FieldClass.CONTENT),
    )
    source_type: str = Field(
        description="webhook | schedule | topic", **classify(FieldClass.CONTENT)
    )
    event_type: str | None = Field(
        default=None,
        description="Topic routing key (e.g. 'ticket.created'); topic sources only",
        **classify(FieldClass.CONTENT),
    )
    organization_id: str | None = Field(
        default=None,
    ...
    is_active: bool = Field(
        default=True,
        description="Enable/disable this source",
        **classify(FieldClass.ENVIRONMENT),
    )
    # Schedule config
    ...
    adapter_name: str | None = Field(
        default=None,
        description="Webhook adapter (e.g. 'generic', 'halopsa')",
        **classify(FieldClass.CONTENT),
    )
    webhook_integration_id: str | None = Field(
        default=None,
        description="Integration UUID for webhook auth",
        **classify(FieldClass.REFERENCE),
    )
    webhook_config: dict | None = Field(
        default=None,
        description="Adapter-specific config",
        **classify(FieldClass.CONTENT),
    )
    ...
    rate_limit_window_seconds: int = Field(
        default=60, description="Window in seconds.", **classify(FieldClass.CONTENT)
    )
    ...
    # Subscriptions
    subscriptions: list[ManifestEventSubscription] = Field(
        default_factory=list,
        description="Workflow subscriptions",
        **classify(FieldClass.CONTENT),
    )
    ...
    def from_row(
        ...
        *,
        ...
        overlap_policy = (
            ...
            if schedule and schedule.overlap_policy
            else None
        )

        adapter_name = webhook.adapter_name if webhook else None
        webhook_integration_id = (
            str(webhook.integration_id) if webhook and webhook.integration_id else None
        )
        ...
        return cls(
            ...
            if isinstance(es.source_type, str)
            else es.source_type.value,
...
class ManifestMCPConnectionTool(EntityCodec, BaseModel):
    ...
    tool_name: str = Field(
        description="Tool name as published by the vendor",
        **classify(FieldClass.CONTENT, match_key=True),
    )
    ...
    enabled: bool = Field(
        default=True,
        description="Whether the tool is enabled in this connection",
        **classify(FieldClass.CONTENT),
    )
    disabled_reason: str | None = Field(
        default=None,
        description="Reason the tool is disabled (admin-set or auto-set)",
        **classify(FieldClass.CONTENT),
    )
...
class ManifestMCPConnection(EntityCodec, BaseModel):
    ...
    organization_id: str = Field(
        description="Organization UUID this connection belongs to",
        **classify(FieldClass.ENVIRONMENT),
    )
    client_id: str = Field(
        description="Vendor-issued OAuth client_id for this org",
        **classify(FieldClass.REFERENCE),
    )
    server_url_override: str | None = Field(
        default=None,
        description="Per-org URL override (regional/sovereign)",
        **classify(FieldClass.CONTENT),
    )
    available_in_chat: bool = Field(
        default=False,
        description="Chat fallback to shared service token when user not connected",
        **classify(FieldClass.CONTENT),
    )
    available_to_autonomous: bool = Field(
        default=False,
        description="Autonomous runs may use shared service token",
        **classify(FieldClass.CONTENT),
    )
    service_oauth_token_id: str | None = Field(
        default=None,
        description="FK to oauth_tokens for shared service token",
        **classify(FieldClass.REFERENCE),
    )
    tools: list[ManifestMCPConnectionTool] = Field(
        default_factory=list,
        description="Per-connection tool catalog",
        **classify(FieldClass.CONTENT),
    )
...
class ManifestMCPServer(EntityCodec, BaseModel):
    ...
    name: str = Field(
        description="Display name (unique)", **classify(FieldClass.CONTENT)
    )
    ...
    oauth_provider_id: str | None = Field(
        default=None,
        description="OAuthProvider UUID; absent for unauthenticated servers",
        **classify(FieldClass.REFERENCE),
    )
    redirect_url: str | None = Field(
        default=None,
        description="Deterministic redirect URL for OAuth callback",
        **classify(FieldClass.CONTENT),
    )
    discovery_metadata: dict | None = Field(
        default=None,
        description="Snapshot of /.well-known payloads at create time",
        **classify(FieldClass.CONTENT),
    )
    organization_id: str | None = Field(
        default=None,
        description="Org UUID (null = platform-level template)",
        **classify(FieldClass.ENVIRONMENT),
    )
    is_active: bool = Field(
        default=True, description="Active flag", **classify(FieldClass.ENVIRONMENT)
    )
    ...
    def from_row(
        ...
        *,
        ...
        return cls(
            ...
            connections={
                cid: ManifestMCPConnection.from_row(
                    conn, tools=tools_lookup.get(cid, [])
                )
...
def filter_manifest_by_ids(manifest: Manifest, entity_ids: set[str]) -> Manifest:
    ...
    return Manifest(
        ...
        integrations={
            k: v for k, v in manifest.integrations.items() if k in entity_ids
        },
        ...
        policy_rules={
            k: v for k, v in manifest.policy_rules.items() if k in entity_ids
        },
...
def validate_manifest(manifest: Manifest) -> list[str]:
    ...
    for _key, wf in manifest.workflows.items():
        ...
        if wf.organization_id and wf.organization_id not in org_ids:
            errors.append(
                f"Workflow '{wf_label}' references unknown organization: {wf.organization_id}"
            )
        for role_id in wf.roles:
            if role_id not in role_ids:
                errors.append(
                    f"Workflow '{wf_label}' references unknown role: {role_id}"
                )
    ...
    for _key, form in manifest.forms.items():
        ...
        if form.organization_id and form.organization_id not in org_ids:
            errors.append(
                f"Form '{form_label}' references unknown organization: {form.organization_id}"
            )
    ...
    for _key, agent in manifest.agents.items():
        ...
        if agent.organization_id and agent.organization_id not in org_ids:
            errors.append(
                f"Agent '{agent_label}' references unknown organization: {agent.organization_id}"
            )
        for role_id in agent.roles:
            if role_id not in role_ids:
                errors.append(
                    f"Agent '{agent_label}' references unknown role: {role_id}"
                )
    ...
    for _key, app in manifest.apps.items():
        ...
        if app.organization_id and app.organization_id not in org_ids:
            errors.append(
                f"App '{app_label}' references unknown organization: {app.organization_id}"
            )
    ...
    for _key, integ in manifest.integrations.items():
        ...
        if (
            integ.list_entities_data_provider_id
            and integ.list_entities_data_provider_id not in wf_ids
        ):
            errors.append(
    ...
    for key, cfg in manifest.configs.items():
        if cfg.integration_id and cfg.integration_id not in integration_ids:
            errors.append(
                f"Config '{key}' references unknown integration: {cfg.integration_id}"
            )
        if cfg.organization_id and cfg.organization_id not in org_ids:
            errors.append(
                f"Config '{key}' references unknown organization: {cfg.organization_id}"
            )
    ...
    for _key, table in manifest.tables.items():
        ...
        if table.organization_id and table.organization_id not in org_ids:
            errors.append(
                f"Table '{table_label}' references unknown organization: {table.organization_id}"
            )
    ...
    for _key, evt in manifest.events.items():
        ...
        if evt.organization_id and evt.organization_id not in org_ids:
            errors.append(
                f"Event source '{evt_label}' references unknown organization: {evt.organization_id}"
            )
        if (
            evt.webhook_integration_id
            and evt.webhook_integration_id not in integration_ids
        ):
            errors.append(
