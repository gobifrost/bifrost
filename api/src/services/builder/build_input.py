...
class UnsupportedDependency(Exception):
    ...
    def __init__(self, offenders: dict[str, str]):
        ...
        detail = ", ".join(
            (
                f"{name}@{version} (use {catalog[name]})"
                if name in catalog
                else f"{name}@{version} (not in the curated catalog)"
            )
            for name, version in sorted(offenders.items())
        )
...
def validate_app_dependencies(
    ...
    """Validate both manifest and source-package dependency declarations.

    This is the dependency half of the fixed build contract, exposed so the
    Builder's ``validate_solution`` tool and the deploy-time materializer use
    the same policy. Toolchain packages are injected by Bifrost, but when a
    source package declares them it must still use the curated pin.
    """
    ...
    validate_dependencies(
        {**source_dependencies, **dependencies, **source_dev_dependencies}
    )
...
def _build_package_json(
    ...
    validate_app_dependencies(src_files, dependencies)

    existing_raw = src_files.get("package.json")
    loaded = json.loads(existing_raw) if existing_raw is not None else {}
