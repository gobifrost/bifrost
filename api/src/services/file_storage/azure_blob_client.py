...
class AzureBlobStorageClient:
    ...
    def _parse_account_name(account_url: str) -> str:
        ...
        path_account = parsed.path.strip("/").split("/", 1)[0]
        if path_account:
            return unquote(path_account)
    ...
    async def _ensure_client(self) -> None:
        ...
        if self.settings.azure_blob_auth == "account_key":
            credential = AzureNamedKeyCredential(
                self._account_name,
                self.settings.azure_blob_account_key or "",
            )
        ...
        *,
        ...
        await self._container_client.upload_blob(
            ...
            content_settings=ContentSettings(
                content_type=content_type or self.guess_content_type(path)
            ),
        )
    ...
    def guess_content_type(path: str) -> str:
        ...
        return S3StorageClient.guess_content_type(path)
