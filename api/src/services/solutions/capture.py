...
class SolutionCaptureService:
    ...
    async def _agent_bundle_files(
        ...
        """Load portable companion files for bundled Agent Skills."""
        ...
        for agent in agents:
            ...
            if scoped_paths:
                ...
                source = self.repo if created_by == "file_sync" else AgentSkillStorage(agent["id"])
                paths = await source.list(prefix)
            if f"{prefix}SKILL.md" not in paths:
                raise ValueError(f"agent bundle is missing SKILL.md: {bundle_path}")
            for path in paths:
