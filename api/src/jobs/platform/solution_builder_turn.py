"""PlatformJob control plane for native Solution Builder turns."""
...
async def run_solution_builder_turn(
    ...
    """Validate the turn, dispatch it, then release scheduler capacity."""
    ...
    await context.report("Starting Builder workspace", percent=1)
    try:
...
async def _fail_turn(turn_id: UUID, message: str) -> None:
    async with get_db_context() as db:
        ...
        if turn is None or turn.status in {"succeeded", "failed", "cancelled"}:
            return
        turn.status = "failed"
...
async def cancel_solution_builder_turn(job_id: UUID, raw_payload: BaseModel) -> None:
    """Synchronize the turn projection and notify the selected executor."""
    from src.models.orm.platform_jobs import PlatformJob

    payload = SolutionBuilderTurnPayload.model_validate(raw_payload)
    await _cancel_turn_projection(payload.turn_id)
    async with get_db_context() as db:
