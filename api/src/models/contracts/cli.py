...
class CLISessionRegisterRequest(BaseModel):
    ...
    selected_workflow: str | None = Field(
        default=None, description="Pre-selected workflow name"
    )
...
class CLIFileWriteRequest(BaseModel):
    ...
    create_only: bool = Field(
        default=False, description="Fail if the file already exists"
    )
...
class CLIConfigSetRequest(BaseModel):
    ...
    is_secret: bool = Field(default=False, description="Whether to encrypt the value")
    scope: str | None = Field(
        default=None,
        description="Organization scope: None=context default, UUID=specific org, 'global'=global scope",
    )
...
class SDKIntegrationsOAuthData(BaseModel):
    ...
    client_secret: str | None = Field(
        None, description="OAuth client secret (decrypted)"
    )
    authorization_url: str | None = Field(None, description="OAuth authorization URL")
    token_url: str | None = Field(
        None, description="OAuth token URL (with {entity_id} resolved)"
    )
    scopes: list[str] = Field(default_factory=list, description="OAuth scopes")
    access_token: str | None = Field(
        None, description="Current access token (decrypted)"
    )
...
class SDKIntegrationsGetResponse(BaseModel):
    ...
    entity_id: str | None = Field(
        None, description="Mapped external entity ID (e.g., tenant ID)"
    )
    entity_name: str | None = Field(
        None, description="Display name for the mapped entity"
    )
...
class SDKIntegrationsMappingItem(BaseModel):
    ...
    oauth_token_id: str | None = Field(
        None, description="Per-org OAuth token override ID"
    )
    config: dict[str, Any] | None = Field(
        None, description="Organization-specific config"
    )
...
class SDKIntegrationsUpsertMappingRequest(BaseModel):
    ...
    entity_id: str = Field(..., description="External entity ID")
    entity_name: str | None = Field(
        default=None, description="Display name for the entity"
    )
    config: dict[str, Any] | None = Field(
        default=None, description="Org-specific configuration"
    )
...
class CLIAICompleteRequest(BaseModel):
    ...
    org_id: str | None = Field(
        default=None, description="Organization ID for knowledge search"
    )
    profile: str | None = Field(
        default=None, description="Model profile name (null = platform default profile)"
    )
    ...
    execution_id: str | None = Field(
        default=None, description="Execution ID for AI usage tracking"
    )
...
class CLIKnowledgeStoreRequest(BaseModel):
    ...
    metadata: dict[str, Any] | None = Field(
        default=None, description="Optional metadata dict"
    )
    scope: str | None = Field(
        default=None,
        description="Organization scope: None=context default, UUID=specific org, 'global'=global scope",
...
class CLIKnowledgeSearchRequest(BaseModel):
    ...
    namespace: list[str] = Field(
        default=["default"], description="Namespace(s) to search"
    )
    limit: int = Field(default=5, description="Maximum results")
    min_score: float | None = Field(
        default=None, description="Minimum similarity score (0-1)"
    )
    ...
    fallback: bool = Field(
        default=True, description="If True, also search global scope"
    )
...
class SDKTableCreateRequest(BaseModel):
    ...
    name: str = Field(
        ..., pattern=r"^[a-z][a-z0-9_]*$", description="Table name (lowercase)"
    )
    table_schema: dict[str, Any] | None = Field(
        default=None, description="Optional schema hints"
    )
...
class SDKTableInfo(BaseModel):
    ...
    organization_id: str | None = Field(
        None, description="Organization UUID or null for global"
    )
