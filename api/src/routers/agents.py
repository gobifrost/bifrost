...
async def _accessible_agent(
    ...
    """Resolve an agent after the private-Solution parent gate.

    Mutation handlers retain their established 403 response for visible,
    non-owned loose agents. ``get_agent`` is intentionally the raw ID lookup
    with the central private-parent criterion applied; the handler's ownership
    check remains the authorization gate for the mutation itself.
    """
...
async def create_agent(
    ...
    if not is_admin:
        ...
        agent_data.role_ids = []
        # Bundle roots grant read access to every companion asset beneath them.
        # Only administrators may bind that capability to an ad-hoc Agent.
...
async def upload_agent_skill(
    ...
    # Runtime compatibility: execution still reads Agent.system_prompt. The
    # bundle's SKILL.md is the canonical source and replaces that materialized
    # value wholesale on every upload.
    agent.system_prompt = imported.skill_markdown
    # An administrator may replace a git-synced bundle interactively. From
    # this point forward the Agent is backed by its isolated upload namespace,
    # not by the git workspace's `_repo/` tree.
