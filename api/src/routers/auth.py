...
from src.services.user_authorization import require_exact_user_boundary
...
@router.get(
    ...
    response_model=AuthorizationTargetsPublic,
    summary="List the current person's selectable authorization contexts",
)
async def list_authorization_targets(
    user: CurrentActiveUser,
    db: DbSession,
) -> AuthorizationTargetsPublic:
    """Bootstrap explicit Organization, Managed, and Global UI context.
    ...
    if user.is_external:
        raise HTTPException(
            ...
            detail="Authorization contexts are unavailable to external sessions",
        )
    from src.services.authorization_targets import discover_authorization_targets

    discovered = await discover_authorization_targets(db, requester=user)
    targets = [
        AuthorizationTargetPublic(
            boundary=f"organization:{organization.id}",
            kind="organization",
            label=organization.name,
            capabilities=sorted(organization.capabilities),
            organization_id=organization.id,
            is_provider=organization.is_provider,
        )
        for organization in discovered.organizations
    ]
    if discovered.managed_capabilities:
        targets.append(
            AuthorizationTargetPublic(
                boundary="managed_organizations",
                kind="managed_organizations",
                label="Managed organizations",
                capabilities=sorted(discovered.managed_capabilities),
            )
        )
    if discovered.platform_capabilities:
        targets.append(
            AuthorizationTargetPublic(
                boundary="platform",
                kind="platform",
                label="Global",
                capabilities=sorted(discovered.platform_capabilities),
            )
        )
    return AuthorizationTargetsPublic(targets=targets)
...
async def admin_revoke_user_sessions(
    ...
    """
    Revoke all refresh tokens for a user in the selected boundary.
    ...
    authorization.require("organizations.readwrite")
    ...
    logger.warning(
        f"Administrator revoked all sessions for user: {log_safe(target_user.email)}",
        extra={
            "admin_user": authorization.effective_actor.email,
