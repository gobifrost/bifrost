...
async def get_scheduler_diagnostics(
    ...
    active_by_owner = {owner: job_id for owner, job_id in active_rows}
    ...
    replicas = [
        SchedulerReplicaStatus(
            ...
            active_platform_job_id=active_by_owner.get(row.id),
        )
    ...
    capacity = SchedulerCapacityStatus(
        ...
        slots_total=len(online_replicas),
        slots_running=sum(1 for replica in online_replicas if replica.active_platform_job_id),
