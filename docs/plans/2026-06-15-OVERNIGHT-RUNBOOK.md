...
- **Commits added overnight (oldest→newest):** `24c64811` (runbook) → `df734d3a` (required-flag enforcement FIX + test) → `2b8e5585` (select-field options) → `0b7959fd` (apps/integrations --org-only) → `5b6c5cc0` (apps --app-model inline_v1) → `0f9a3c40` (--tool-ids @tool) → `7d6d1980` (Track B accept) → `0b6293ab` (README git-guard + e2e) → `c630ea6b` (SolutionDetail 9→3 redesign) → (+ runbook/doc commits between).
