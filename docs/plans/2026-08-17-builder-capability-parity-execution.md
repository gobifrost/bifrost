...
- [x] Rewrite authored Skill guidance around stable operation intent, target
      selection, build lifecycle, safety, and verification. (SKILL.md now opens
      with "Finding the right operation" — intent IDs, not commands — and every
      product rule is stated transport-neutrally. 165 -> 149 lines with more
      content preserved.)
- [x] Remove handwritten lists that teach separate CLI/MCP/Builder dialects.
      (Deleted the Native Builder section, the CLI Prerequisites mechanics, and
      the stale "requires CLI 1.2.2" claim — the real floor was already 1.2.3
      and the server publishes it via /api/version. references/mcp-mode.md lost
      its hardcoded tool-name list, 90 -> 60 lines, keeping every product rule.)
- [x] Retain only a small bootstrap rule: use dynamic capability discovery when
      present; otherwise load the generated CLI binding. (Three ordered steps:
      bifrost_search_capabilities for a live schema, else generated/operations.md
      and cli-reference.md, never infer from memory.)
- [x] Generate operation and CLI appendices from the catalog and keep CI
      freshness enforcement. (Already generated by scripts/skill-truth and
      scripts/operation_catalog; the parity-boundary claim in mcp-mode.md now
      DEFERS to generated/operations.md instead of enumerating domains, so it
      cannot go stale.)
- [x] Verify `bifrost skill update` installs the same Skill and references for
      Codex, Claude, Copilot, Cursor, Gemini, and other Agent-Skills-compatible
      harnesses. (Verified in api/bifrost/skill.py: writes .claude/skills/ and
      the cross-harness .agents/skills/ convention, wiping stale state. Added a
      test that nested references/ and generated/ trees land in BOTH locations —
      the Skill is useless if its routed files do not travel with it.)
