...
> **COMPLETE orphan-reference removal (L1 dropped the columns; these sites now reference dropped columns → runtime AttributeError unless removed). This task removes ALL of them so the tree is runtime-clean.** Grep-verified inventory (do NOT touch `workflow_orphan.py` / `routers/workflows.py` / `contracts/github.py` — that's a SEPARATE "deleted-file workflow" orphaning concept via `Workflow.is_orphaned`, which never had these columns; LEAVE it entirely):
> - **`api/src/routers/websocket.py`** lines ~106, ~215: `Table.orphaned_at.is_(None)` filter in by-name table resolution → DELETE the filter clause (no orphaned rows exist anymore).
> - **`api/src/services/solutions/capture.py`** line ~754: `Config.orphaned_at.is_(None)` filter when capturing config → DELETE the filter.
> - **`api/src/repositories/config.py`**: filters at ~84-85, ~213, ~231, ~274 (`orphaned_at.is_(None)`) → DELETE each; heal-assignments at ~316-318 (`existing.orphaned_at = None; origin_solution_slug = None; origin_solution_id = None`) → DELETE; the `orphaned_at`/`origin_solution_slug` reads in the DTO-serialization at ~127-128 → DELETE.
> - **`api/src/repositories/tables.py`**: `include_orphaned` param (~28) + `orphaned_at.is_(None)` filter (~47) → DELETE both. (This is the L7 work; doing it here is fine — just remove it from L7's scope when you get there, or L7 confirms it's gone.)
> - **`api/src/repositories/org_scoped.py`**: `_model_has_orphaned_at` helper (~49-50) + its two call-site filters (~252-253, ~342-343) + the `include_orphaned` param → DELETE (defensive dead code now).
> - **`api/src/services/solutions/deploy.py`**: the orphaned-TABLE reattach query/block (~955-988, references `Table.orphaned_at`/`origin_solution_slug`) and the orphaned-CONFIG reattach (~1798-1801) → DELETE the reattach blocks. (This is L5's work; doing it here is fine, or L5 confirms gone.) Also `_reattach_orphan_configs` method + its call.
> - **`api/src/services/solutions/zip_install.py`**: line ~621 (`Config.orphaned_at.is_(None)` filter) + the orphaned-table reattach query (~657-669) → DELETE.
> - **`api/src/routers/config.py`** (~51-53) + **`api/src/routers/tables.py`**: the `include_orphaned` Query param + passthrough → DELETE. (L7 work; do here or confirm at L7.)
> - **Contract DTOs (cosmetic, no runtime break, but remove for cleanliness + regen types):** `api/src/models/contracts/config.py` (~38-45: `orphaned_at` + `origin_solution_slug` fields), `api/src/models/contracts/tables.py` (~101-102: same two fields). DELETE these fields. Then regenerate client types.
>
> After all removals: `grep -rn "orphaned_at\|origin_solution_slug\|origin_solution_id" api/src --include=*.py | grep -v solution_file_jobs | grep -v workflow_orphan | grep -v contracts/github` must return NOTHING (the only surviving `origin_solution_id` is on `SolutionFileJob` ORM/contract, which is unrelated and stays unless the job itself is deleted in Step 2). Run `cd api && ruff check .` + `cd api && pyright` to confirm no dangling references. Since this widens L2 well beyond "delete the file service," it is acceptable for L2 to also touch deploy.py/zip_install.py/repos/routers/contracts; note in your report which sites you removed so L5/L7 know they're already done.
