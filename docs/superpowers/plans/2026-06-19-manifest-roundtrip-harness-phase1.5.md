...
- [ ] **Step 3: Run — green for ALL 20.** `./test.sh tests/roundtrip/test_generator_completeness.py -v`. If a model fails validation (e.g. an aliased field like `ManifestTable.table_schema` alias `schema`, or a `ClaimQuery` nested type), fix `sentinel_for`/validation call to handle that type — do NOT skip the field. Note `populate_by_name` models accept the python name; for alias-only models pass `by_alias` appropriately.
  ...
  **DOMAIN-VALID sentinels (Codex round-2 P1 + round-3):** some string fields are enum-constrained DOWNSTREAM even though the manifest types them as `str` — they pass `model_validate` but a raw `SENT::...` string breaks the real import/deploy path. Maintain a small `DOMAIN_VALUES` override map in `generators.py` for these, returning a valid member: `access_level` → `"role_based"`; agent `channels` → `["chat"]` (filtered to valid enums at `agent.py:100`); EventSource `source_type` → `"schedule"` (DB enum `events.py:50`); `config_type` → `"string"` (so the `Config.value` predicate stays CONTENT in the base fixture; a SECOND fixture sets `config_type="secret"` to exercise the SECRET branch — see Task 6); **`app_model` → `"standalone_v2"`** (Solution deploy REJECTS non-standalone_v2 apps, `deploy.py:1095-1103`); **`overlap_policy` → `"skip"`** (deploy casts through `ScheduleOverlapPolicy`, `enums.py:156`, `deploy.py:1594`). `sentinel_for` consults `DOMAIN_VALUES[(model_name, field)]` (or `[field]`) before falling back to a generic sentinel. This is the all-fields-populated path; the round-trip tests (Tasks 5-6) need values that survive the REAL import, not just pydantic validation. (Codex round-3 verified all 20 models pass `model_validate` after the PEP-604 fix; this map is for the deeper REAL-import survival.)
...
pass `model_validate` and all round-2 structural fixes landed. These remaining items are concrete and
each cites the code line — apply them in the named task:

- **PER-FIELD TRANSFORM POLICIES (Task 4/6) — some fields are neither pure keep nor pure solution-remap:**
  - `mcp_connection_ids` (Agent): env-scoped grant ids, **NOT** solution-remapped (`deploy.py:1340-1348`). The generic list-remap assertion would FALSE-RED. Give it a per-field policy: assert keep-as-env-ref (presence + value preserved), not `solution_entity_id` remap.
  - `form_schema` (Form): not pure keep — its nested `data_provider_id` IS remapped (`capture.py:977`, `deploy.py:599-610`). Compare with the nested ref remapped, not byte-equal.
  - `webhook_integration_id` (EventSource): `_repo` reference, but Solution deploy intentionally drops it to `integration_id=None` (`manifest_generator.py:334`, `deploy.py:1609-1618`). Solution policy for this field = scrub/drop, not remap.
  - Mechanism: a small `FIELD_OVERRIDES[(entity, field)] -> action` map consulted before the class default; every entry cites the code line proving the transform (this is also the Task-7 guardrail — no override without a citation).
- **`EXTRA_FIELD_POLICY` starter list (Global Constraints / Task 6) — extend beyond `max_run_timeout`:** app `repo_path`/`logo_b64`/`logo_content_type`/`src_files`/`bin_files`/`dist_files`/`bin_dist_files` (`capture.py:537-553`, `export.py:42-49`); form `workflow_path`/`workflow_function_name` (`capture.py:579-581`); workflow collector `source` (`commands/solution.py:772-780`); connection-decl `integration_name`/`template`/`position` (`export.py:135-148`). NOTE: `schema` is NOT an extra — it's `ManifestTable.table_schema`'s alias (`manifest.py:492`); handle via alias, not EXTRA_FIELD_POLICY.
- **`_repo` driver return shape (Task 5):** `_import_all_entities` returns `(count, entity_changes)` (`github_sync.py:1188-1234`), NOT SyncOps. Assert the delta via `count > 0` / the entity appearing, not "non-empty SyncOps." Also `has_entities` omits `claims`/`mcp_servers` and `_diff_and_collect` omits `mcp_servers` (`github_sync.py:1151`, `manifest_import.py:82`) — so the `_repo` MCP-server round-trip goes through a DIFFERENT path; cover MCP servers via the direct resolver, and note claims need the entity present to import.
- **DEPENDENCY CLOSURES for fixtures (Task 3/5):** `each_field_isolated`/`all_fields_populated` must seed referenced entities + real files or the REAL import skips the row (not a drop — a fixture gap → noise red): workflow import checks file existence (`manifest_import.py:662-760`, `:687-699`); export filters workflows/apps by file/dir (`github_sync.py:704-716`); agent inline import needs the org/refs present (`manifest_import.py:247-268`, indexer skips missing refs `agent.py:166-174`). The generator must build a valid dependency closure (org→role→integration→workflow→form/agent) + write the referenced `.py`/app files, OR the test seeds them. Add this as an explicit fixture-builder step before the round-trip assertion.
- **SPEC SYNC (do in Task 2 commit):** spec v2 is stale on two points the plan already corrected — `access_level` shown as environment (`spec:189-192`; should be CONTENT) and the predicate shown as a callable `when` in `json_schema_extra` (`spec:70-81`; should be the string-key registry). Update both spec lines so spec and plan agree.
