...
sampled worker reported a cgroup working set of 573,431,808 bytes (547 MiB) and
a historical cgroup peak of 962,879,488 bytes (918 MiB). Its parent process
...
additive accounting; cgroup working set is the authoritative pod measure.

One sampled scheduler reported a cgroup working set of 185,569,280 bytes and a
historical peak of 481,406,976 bytes. Recent SDK-update PlatformJobs also showed
...
Workers already set RabbitMQ prefetch to their configured internal concurrency.
That relationship must remain bounded so a pod cannot reserve a large invisible
backlog. When all admitted slots are occupied, ready messages remain in the
queue and KEDA adds replicas.
...
KEDA `ScaledJob` resources provide one-pod-per-message execution for the
`isolated` and initially the `high-memory` lanes. Each pod:

1. starts the lean `run-one` entry point;
2. consumes or is assigned one envelope;
...
The pod template declares a resource request and limit from the work's resource
class. A pod cannot be resized to match a three-megabyte application heap: the
Python interpreter, imported Bifrost runtime, allocator, and system libraries
establish a real baseline. The goal is to measure and minimize that baseline,
then request baseline plus predicted workload memory and safety margin rather
than reserving the full current worker size.
...
- one database/RabbitMQ/Redis lifecycle bounded to one execution;
- cgroup-v2 current and peak working-set reporting;
...
- lean one-shot runner pre-workload working set at or below 128 MiB;
...
The existing package-install broadcast is incompatible with scale-to-zero:
when no worker subscriber exists, an exclusive auto-delete broadcast queue
cannot retain the update for a future pod. Ephemeral pods also cannot depend on
mutations made to a previous pod filesystem.
...
artifact during startup. A dependency-set fingerprint becomes part of pool and
Job identity. Policy must not activate a pool whose dependency fingerprint does
not match the work it may consume.

The spike inventories this boundary and measures startup hydration, but it does
not redesign package distribution completely.
...
2. Reconcile and validate every new KEDA resource, Deployment, Job template,
   ServiceAccount, and queue consumer required by that generation.
3. Confirm that each lane has at least one valid capacity provider, including
   scale-from-zero providers.
4. Atomically mark the generation active in PostgreSQL.
5. Publish a Redis `execution_policy_changed` notification containing only the
   generation identifier.
6. Schedulers and workers fetch authoritative policy, stop accepting new
   deliveries for relinquished lanes, and drain in-flight work.
7. Executors report their applied generation through renewable heartbeats.
8. The controller marks the policy converged after old ownership has drained.
...
- Queue pressure above the pool's admitted internal concurrency adds replicas.
- Background and isolated capacity may remain at zero.
- Scale-down drains consumers within the existing termination grace period.
...
- KEDA creates a Job pod with the class request and limit.
...
| User cancels work | Stop new delivery, delete or terminate the owning Job/pod, and fence late completion. |
| Scale-down begins | Cancel RabbitMQ consumer tags, drain in-flight work, then exit within the pod grace period. |
...
2. Scale the current worker Deployment from zero on the existing
   `workflow-executions` RabbitMQ queue.
...
5. Drain and scale down without losing or duplicating work.
...
- execution duration;
- cgroup current and peak working set;
...
### Experiment 1: Existing worker elasticity

Use the current RabbitMQ workflow queue and worker image unchanged except for
test observability. Prove zero-to-one, one-to-many, scale-down, and warm-latency
behavior with KEDA. This establishes whether the external mechanism works
before adding Bifrost control code.

### Experiment 2: Lean one-shot runner

Extract the smallest executable path capable of running one deterministic
workflow and exiting. Attribute its import and runtime memory, compare it with
the current parent/template topology, and decide whether strict import hygiene
or a separate image build target is warranted.

### Experiment 3: Isolated PlatformJob

Add the minimum internal execution classification and Kubernetes runner seam
needed to execute one SDK-update PlatformJob in a resource-limited Job pod.
Keep PlatformJob as the durable owner and verify status, progress, cancellation,
fencing, timeout, OOM, and cleanup.

### Experiment 4: Policy reconciliation

Prototype a versioned runtime policy and demonstrate prepare, activate, notify,
drain, and converge transitions. This experiment may use an administrative
test endpoint or fixture rather than production UI.
...
- safe scale-down, retries, cancellation, and controller recovery;
