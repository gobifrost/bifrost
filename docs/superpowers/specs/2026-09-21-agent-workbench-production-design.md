...
**Status:** Revised layout approved on 2026-09-21; pending written-spec review.
...
3. A compact, icon-led navigation rail groups collections by operator intent:
   `Findings` under Act, `Tests` under Validate, `Reviews` and scheduling under
   Automate, and `Run History` under Evidence. Test status filters such as `All
   Tests`, `Failing`, and `Not Run` remain grouped beneath Tests. Group labels,
   spacing, icons, and counts expose these roles without turning the rail into
   a dashboard.
...
5. Full-width, collection-specific rows show identity first, then concise
   status, provenance, profile, time, duration, and cost when relevant. A
   Finding, Test, and Review remain distinguishable during a quick scan.
...
The collection is the primary work surface. With no selection, it consumes all
space remaining after the compact navigation rail. The inspector is absent,
not reserved as an empty column.

On wide screens, selecting a row attaches an inspector that uses approximately
36% of the workspace, with the collection retaining at least 52% after the
navigation rail. At intermediate widths the inspector replaces the collection
instead of squeezing it into an unusable center column. On phones, collection
navigation collapses into the workspace header and the inspector is a focused
pane with an explicit return action that restores collection state.

Search, high-frequency filters, selection context, and the primary action share
one responsive toolbar. They may wrap by priority, but must not default to a
stacked column on desktop. Close-inspector behavior uses the standard icon-led
workspace affordance rather than a visually dominant text band.

### Visual language

The Workbench is an operational surface, but not a generic CRUD shell. It uses
the existing Lucide icon family, icon-led section headings, navigation selection
classes, semantic outcome icons, restrained collection accents, and tonal
surface changes from the modernized Bifrost pages.

Icons reinforce labels rather than replacing them. Findings emphasize kind and
source, Tests emphasize latest verdict and readiness, Reviews emphasize cadence
and generated output, and Run History emphasizes execution lifecycle. Hover,
focus, pressed, and selected treatments span the row and remain visually
distinct. Visual separation comes first from proximity, rhythm, and dividers;
ordinary content is not broken into nested cards.
...
The collection is the primary fleet inbox. A source badge and icon distinguish
Findings generated by a Review, failed Test, run triage, or manual entry. Review-
generated Findings link back to the Review so the causal relationship is visible
without presenting Reviews and Findings as equivalent records.
...
Reviews are automation definitions, not inbox items. Their collection header
explains that they inspect completed runs and surface Findings. Rows and the
inspector prioritize cadence, scope, last execution, and generated Finding
count rather than borrowing the Finding row treatment.
